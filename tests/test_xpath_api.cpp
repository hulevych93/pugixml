...
TEST_XML(xpath_api_node_set_assign_out_of_memory_preserve, "<node><a/><b/></node>")
{
	...
	CHECK(ns.size() == 2);
	CHECK(ns.type() == type_sorted);

	xpath_node_set nsall = doc.select_nodes(STR("//*"));
	nsall.sort(true);
	CHECK(nsall.size() == 3);
	CHECK(nsall.type() == type_sorted_reverse);
...
TEST_XML(xpath_api_nodeset_move_ctor, "<node><foo/><foo/><bar/></node>")
{
	...
	CHECK(set.size() == 2);
	CHECK(set.type() == type_sorted_reverse);
	...
	CHECK(set.size() == 0);
	CHECK(set.type() == type_unsorted);

	CHECK(move.size() == 2);
	CHECK(move.type() == type_sorted_reverse);
...
TEST_XML(xpath_api_nodeset_move_assign_destroy, "<node><foo/><foo/><bar/></node>")
{
	...
	CHECK(all.size() == 2);
	CHECK(all.type() == type_sorted_reverse);
